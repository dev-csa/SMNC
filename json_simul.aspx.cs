...
public partial class json_simul : System.Web.UI.Page   // Simulation
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (v2 == "screen") { // Screen 시뮬  <TV PC MO>
        ...
        if (getcost1 + getcost2 + getcost2 != "0") {        
            ...
            if(v2 == "digital"){
                ...
                grp1 = iuserInput1 / cprpcpm1;    // TV            
                imp1 = grp1 / 100;
            }

            if(v2 == "tvdigital" && tvType == "cable"){                
                grp2 = iuserInput2 / cprpcpm2;    // 케이블&종편
                imp2 = grp2 / 100;
                //Response.Write(iuserInput2 + ";" + cprpcpm2);
            }
            else{
            ...
            // TOTAL                
            double R1_total = R1_sim1 + R1_sim2 + R1_sim3 - ratio_12*(R1_sim1 * R1_sim2) - ratio_13*(R1_sim1 * R1_sim3) - ratio_23*(R1_sim2 * R1_sim3) + ratio_123*(R1_sim1 * R1_sim2 * R1_sim3);
            R1_total = Math.Min(R1_total, 0.99);
            double R3_total = R3_sim1 + R3_sim2 + R3_sim3 - (R3_sim1 * R3_sim2) - (R3_sim1 * R3_sim3) - (R3_sim2 * R3_sim3) + (R3_sim1 * R3_sim2 * R3_sim3);
            R3_total = Math.Min(R3_total, R1_total);
            double GRP_total = grp1 + grp2 + grp3;
            double AF_total = GRP_total / R1_total / 100;
